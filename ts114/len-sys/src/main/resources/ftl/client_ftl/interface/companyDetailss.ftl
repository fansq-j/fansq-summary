...
<div id="zhuye">
    ...
    <div style="height: 100%;" id="Huan">
        <#list serverClientRes as itemRes>
            <dl>
                <dt><h2 style="font-size: 20px;text-align: center;">${itemRes.gsxx_name!}</h2></dt>
                <dd style="line-height: 50px;color: #403b3b;font-size: 15px;text-indent:2em;">${itemRes.gsxx_intro!}</dd>
                <span>${itemRes.gsxx_create_time?substring(0,10)!}</span></br>
                <span style="line-height: 26px;"><a href="tel:${itemRes.gsxx_tel!}">${itemRes.gsxx_tel?substring(0,3)}*****${itemRes.gsxx_tel?substring(7,11)}    拨打电话</a></span>
            </dl>
        </#list>
    </div>
         ...
         $.ajax({
            ...
            success:function(data){
                ...
                $.each(data,function(index,obj){
                    $("#Huan").append("<div class=\"lieb\"><ul><li><a href=\"${re.contextPath}/CompanyDetails?keyword="+obj.gsxx_name+"&gsxx_id="+obj.gsxx_id+"&num=0\"><h1><img src=\"${re.contextPath}/ftl/client_ftl/images/lieb_img_1.jpg\"></h1><dl><dt><h2>"+obj.gsxx_name+"</h2></dt><dd>"+obj.gsxx_intro+"</dd><span>"+obj.gsxx_create_time+"</span></dl></a></li></ul></div>")
                });
                if (gjz != null && gjz!=""){
                    if (data != null && data != ""){
